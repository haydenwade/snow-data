          ...
          <table className="min-w-max text-sm">
            <thead className="sticky top-0 z-10 bg-slate-800/70 backdrop-blur-sm">
              <tr className="border-slate-700/50">
                <th className="text-left font-medium py-2 pl-4 text-slate-400 w-36 whitespace-nowrap">Date</th>
                <th className="text-right font-medium py-2 text-slate-400 w-32 whitespace-nowrap">Snow</th>
                <th className="text-right font-medium py-2 text-slate-400 w-24 whitespace-nowrap">PoP</th>
                <th className="text-right font-medium py-2 pr-4 text-slate-400 w-36 whitespace-nowrap">Temp ({unit === "mm" ? '°C' : '°F'})</th>
                <th className="text-center font-medium py-2 text-slate-400 w-40 whitespace-nowrap">Wind ({unit === "mm" ? 'kph' : 'mph'})</th>
                <th className="text-center font-medium py-2 text-slate-400 w-36 whitespace-nowrap">Sky Cover</th>
              </tr>
            </thead>
            <tbody>
              {data.map((d, idx) => (
                <tr key={d.date} className="border-t border-slate-700/30 hover:bg-slate-700/20">
                  ...
                  <td className="py-2 text-right w-32 whitespace-nowrap"><SnowCell valueInInches={d.snowIn} unit={unit} tone="forecast" /></td>
                  ...
                  <td className="py-2 text-center text-slate-400 text-sm w-40 whitespace-nowrap">
                    {d.windMph != null ? (
                      unit === "mm" ? (
                        <span className="font-medium">{Math.round(d.windMph * 1.60934)} kph {degToCompass(d.windDirDeg)}</span>
                      ) : (
                        <span className="font-medium">{d.windMph} mph {degToCompass(d.windDirDeg)}</span>
                      )
                    ) : (
                  ...
                  <td className="py-2 text-center text-slate-400 text-sm w-36 whitespace-nowrap">
                    {d.skyCoverPercent != null ? (
                      <span className="font-medium">{skyCoverLabel(d.skyCoverPercent)}</span>
                    ) : (
                      <span className="text-slate-500">—</span>
                    )}
                  </td>
